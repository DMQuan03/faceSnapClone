...
const HEADERSEARHUSER  = () => {
    ...
    const { avatar, token, userId } = sessionStorage
    ...
    const showCommentVideo = useSelector(state => state.video.showMore)
        ...
        axios({
          method : "get",
          url : process.env.REACT_APP_API24 + searchUserTrue + "&limit=" + limit + "&limitBlog=" + limitBlog + "&limitVideo" + limitVideo,
          headers : {
  ...
  return (
    <div className={cx("wrapper")}>
        ...
        <div className={cx("info_user")}>
            ...
            }} className={cx("infoUser")} src={avatar} alt='a' />
            <button className={cx("btn_logout")} 
            onClick={() => {
                navigate("/")
